...
import com.makentoshe.booruchan.screen.posts.container.model.getItemsCountInColumnF
import com.makentoshe.booruchan.screen.posts.container.model.getItemsCountInRowF
...
class PostPageUiGrid : AnkoComponent<_RelativeLayout> {
    override fun createView(ui: AnkoContext<_RelativeLayout>) = with(ui.owner) {
        val verticalSpace =
            (getItemsCountInColumnF(context) - getItemsCountInColumnF(
                context
            ).toInt()) /
